...
- `GATEWAY_CAPABILITY_CACHE_PATH=` — 网关能力缓存落盘路径；扩展启动代理时会自动指向用户配置目录
- `TOOL_DENYLIST=` / `TOOL_DENY_PREFIXES=mcp3_` — 禁止转发指定工具或前缀，减少大 MCP 工具集带来的首包延迟
- `TOOL_ALLOWLIST=` / `TOOL_ALLOW_PREFIXES=` — 只转发指定工具或前缀；支持 `mcp1_*` 这类通配前缀
